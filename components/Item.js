...
const Item = ({ token }) => {
  ...
  return (
    <div className="flex  flex-col justify-between bg-black border-[6px] h-full border-gray-500 rounded-3xl p-5 text-white text-center">
      <h2>{token.name}</h2>
      <img
        onClick={openModal}
      ...
      />
      {showSpinner ? <Spinner /> : ''}
      <div><span>User Address:</span>
      <br />
      <span className="text-xs">{token.user}</span>
      </div>
      ...
      >
        <div className="bg-black border-4 xl:h-auto border-gray-500 rounded-3xl">
          {/* <button className="float-right text-white" onClick={closeModal}>
            X
          </button> */}
